...
function submit() {
    form.post(route('block-roles.store'));
}
</script>

<template>
    <AuthenticatedLayout>
        <Head title="Create Block Role" />
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 animate-fade-in">
                <div>
                    <div class="flex items-center gap-3">
                        ...
                        <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Create Block Role</h1>
                    </div>
                    <p class="mt-2 text-sm text-slate-500 ml-12">Add a new block role.</p>
                </div>
            </div>
                ...
                <div class="card overflow-hidden">
                    <div class="px-8 py-5 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <h2 class="text-base font-semibold text-slate-800">Role Information</h2>
                        </div>
                    </div>

                    <div class="p-8 grid grid-cols-1 gap-6">
                        <div>
                            <InputLabel for="name" value="Role Name *" class="label" />
                            <TextInput id="name" type="text" class="mt-1" v-model="form.name" required placeholder="e.g. Admin" />
                            <InputError class="mt-1.5" :message="form.errors.name" />
                        </div>
                    </div>

                    <div class="flex items-center justify-end gap-4 pt-4 pb-12 pr-8">
                        <Link :href="route('block-roles.index')" class="btn-secondary btn-lg">
                            Cancel
                        </Link>
                        <button type="submit" :disabled="form.processing" class="btn-primary btn-lg min-w-[160px]">
                            ...
                            <span>{{ form.processing ? 'Saving...' : 'Create Role' }}</span>
                        </button>
                    </div>
                </div>
            </form>
