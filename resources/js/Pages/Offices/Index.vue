...
const deleteModal = ref({ show: false, office: null });
const deleteForm = useForm({});

function confirmDelete(office) {
    deleteModal.value = { show: true, office };
}

function destroyOffice() {
    if (!deleteModal.value.office) return;

    deleteForm.delete(route('offices.destroy', deleteModal.value.office.id), {
        preserveScroll: true,
        onSuccess: () => {
            deleteModal.value = { show: false, office: null };
        },
    });
}
</script>
...
<template>
    <AuthenticatedLayout>
        ...
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-lg font-semibold text-slate-800">Offices</h1>
            </div>
            <Link :href="route('offices.create')" class="btn btn-primary">
        ...
        <div class="card overflow-hidden">
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="office in offices.data" :key="office.id">
                            <td>
                                <span class="font-mono text-xs font-semibold">{{ office.id }}</span>
                            </td>
                            ...
                            <td>
                                <div class="flex gap-1">
                                    ...
                                    <button @click="confirmDelete(office)"
            ...
            <!-- Empty State -->
            <div v-if="!offices.data || offices.data.length === 0" class="text-center py-16">
                ...
                <p class="text-sm font-medium text-slate-500">No offices found</p>
                <p class="text-xs text-slate-400 mt-1">Add a new office to get started</p>
            </div>

            <!-- Pagination -->
            <div class="flex items-center justify-between px-5 py-3 border-t border-slate-100">
                <p class="text-xs text-slate-400">
                    Showing {{ offices.from ?? 0 }} to {{ offices.to ?? 0 }} of {{ offices.total ?? 0 }} offices
                </p>
                <Pagination v-if="offices && offices.links" :links="offices.links" />
            </div>
        </div>
    </AuthenticatedLayout>
    ...
    <Modal
        ...
        @close="deleteModal = { show: false, office: null }"
    >
        <div class="p-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <h2 class="text-base font-bold text-slate-800">Delete Office</h2>
            ...
            <p class="text-sm text-slate-600 mb-6 p-3 rounded-xl bg-red-50 border border-red-100">
                ...
                <span class="font-semibold text-red-600">{{ deleteModal.office?.name }}</span>?
            </p>
            <div class="flex justify-end gap-3">
                <SecondaryButton @click="deleteModal = { show: false, office: null }">
                    Cancel
                </SecondaryButton>
                <DangerButton
