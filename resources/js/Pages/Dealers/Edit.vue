...
const form = useForm({
    ...
    dealer_phone: props.dealer.dealer_phone ?? '',
    cnic: props.dealer.cnic ?? '',
    dealer_state: props.dealer.dealer_state ?? '',
    is_parse_data: props.dealer.is_parse_data ?? false,
    Authorize: props.dealer.Authorize ?? true,
    dealer_relation: props.dealer.dealer_relation ?? '',
    dealer_father: props.dealer.dealer_father ?? '',
});
...
function submit() {
    form.put(route('dealers.update', props.dealer.id));
}
</script>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 animate-fade-in">
                <div>
                    <div class="flex items-center gap-3">
                        ...
                        <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Edit Dealer</h1>
                    </div>
                    <p class="mt-2 text-sm text-slate-500 ml-12">Updating profile for <span class="font-medium text-slate-700">{{ props.dealer.name }}</span>.</p>
                </div>
                
                <div class="flex items-center gap-3 mt-4 sm:mt-0">
                    <div class="flex flex-col items-end px-4 py-2 rounded-xl bg-white shadow-sm border border-slate-100">
                        <span class="text-xs font-semibold text-slate-400 uppercase tracking-widest">Dealer ID</span>
                        <span class="text-lg font-bold text-slate-800">#{{ props.dealer.id }}</span>
                    </div>
                </div>
            </div>
                ...
                <div class="card overflow-hidden">
                    ...
                    <div class="flex items-center justify-end gap-4 pt-4 pb-12 pr-8">
                        ...
                        <button type="submit" :disabled="form.processing" class="btn-primary btn-lg min-w-[160px]">
                            ...
                            <span>{{ form.processing ? 'Saving...' : 'Update Dealer' }}</span>
                        </button>
                    </div>
                </div>
            </form>
